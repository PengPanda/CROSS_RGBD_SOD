...
class Data(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.cfg.mode=='train':
            mask  = cv2.imread(self.cfg.datapath+'/train_masks/' +name+'.png', 0).astype(np.float32)
            edge_mask = cv2.imread(self.cfg.datapath+'/train_erode_edges/' +name+'.png', 0).astype(np.float32)
